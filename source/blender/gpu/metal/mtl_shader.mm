    ...
    const MTLRenderPipelineStateDescriptor &pipeline_descriptor)
{
  ...
  @autoreleasepool {
    ...
    /* Reclaim the reserved-but-unused null-attribute-buffer slot for shaders that would otherwise
     * overflow the Metal buffer bind table. `MTL_uniform_buffer_base_index` is
     ...
     * (e.g. an NPR face material with num_vert_buffers=16 and 11 UBOs puts the sampler argument
     * buffer at buffer(31)).
    ...
    /* After the reclaim above, warn if a shader's sampler argument buffer still exceeds the
     * buffer bind table (overflow of more than one slot, or the null-attribute slot was in use so
     * it could not be reclaimed). Such a shader fails to compile and its objects will not render;
     * it needs the material's vertex-attribute / UBO count reduced. */
    if (mtl_interface->uses_argument_buffer_for_samplers()) {
      ...
      if (arg_abs >= MTL_MAX_BUFFER_BINDINGS) {
        MTL_LOG_WARNING(
            "Shader '%s': sampler argument buffer index %d exceeds limit %d and could not be "
    ...
    if (error) {
      ...
      const char *errors_c_str = [[error localizedDescription] UTF8String];
      /* NOTE: `shd_builder_` is deleted at the end of finalize(); it is null whenever
    ...
    if (transform_feedback_type_ == GPU_SHADER_TFB_NONE) {
      ...
      if (error) {
        ...
        const char *errors_c_str = [[error localizedDescription] UTF8String];
        /* NOTE: see the vertex branch above, `shd_builder_` is null at bake time, so guard
         * against dereferencing it for the source text. */
        const StringRefNull source = (shd_builder_ != nullptr) ?
